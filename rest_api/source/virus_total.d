...
override:
    ...
    Json addUser(string userEmail, string username, string password, string name = "", string desc = "")
    {
        ...
        switch (res)
        {
            case DBConnection.UserRet.OK:
                return format("User %s with email %s was added to the database. Please login.", username, userEmail)
                .serializeToJson();

            case DBConnection.UserRet.ERR_INVALID_EMAIL:
                throw new HTTPStatusException(
                    HTTPStatus.badRequest, format("[Error] User email %s is invalid", userEmail)
                    );
            ...
            default:
                throw new HTTPStatusException(
                    HTTPStatus.internalServerError, "[Internal Server Error] user action not defined"
                    );
        }
    }
    ...
    Json addUrl(string userEmail, string urlAddress)
    {
        ...
        final switch (res)
        {
            ...
            case DBConnection.UrlRet.URL_EXISTS:
                ...
                return format(
                    "URL %s is already present in the database. Please see bellow what we already know about it\n%s",
                              urlAddress, url.serializeToPrettyJson()
                    ).serializeToJson();
    ...
    Json getUrlInfo(string urlAddress)
    {
        ...
        {
            throw new HTTPStatusException(
                HTTPStatus.notFound, format("[Error] URL address %s does not exist in the database", urlAddress)
                );
        }
    ...
    Json addFile(string userEmail, immutable ubyte[] binData, string fileName)
    {
        ...
        final switch (res)
        {
            ...
            case DBConnection.FileRet.FILE_EXISTS:
                ...
                return format(
                    "File %s is already present in the database. Please see bellow what we already know about it\n%s",
                    fileName, file.serializeToPrettyJson()
                    ).serializeToJson();
    ...
    Json getFileInfo(string fileSHA512Digest)
    {
        ...
        if (file.isNull)
        {
            throw new HTTPStatusException(
                HTTPStatus.notFound,
                format("[Error] file with SHA512 digest %s does not exist in the database", fileSHA512Digest)
                );
        }
